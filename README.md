...
- 与上次快照对比，**只记录减少项**
- 入口嵌入 **QQ 联系人列表底部**（仿 QNotified 历史好友），无全局悬浮窗
- 在进程内面板查看历史 / 搜索 / 筛选未读 / 导出
- 长按记录尝试打开对应聊天页（查看本地会话，若仍存在）
- 可选系统通知（**默认关闭**）
- 可选定时检查（默认关闭；首次刷新建基线后可设，间隔含「半天」）
- **多账号**：按 QQ 号隔离快照与历史
- **ark-ui 设计语言**：近黑工业仪表风，方形几何 + 信号青

## 更新日志

- **v0.3.1** — 定时按钮标签明确化（「定时：关闭」）；启动器底部新增作者/仓库/GitHub 跳转
- **v0.3.0** — 全面 ark-ui 重设计；移除全局悬浮窗，入口改为联系人列表底部注入（仿 QNotified）；自定义加载指示器/选择器/确认对话框替代原生组件；新增定时检查「半天」间隔 + 基线限制；新增应用图标
- v0.2.1 — 初版进程内面板 + 悬浮窗入口
...
4. **强制停止 QQ** 后重新打开（必须）  
5. 打开入口（任选其一）：
   - **QQ「联系人」页**列表底部 **「单向好友」** 入口（推荐，仿 QNotified）
